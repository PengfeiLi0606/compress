...
c create a 2D table of values if they are not aleady in a .dft file
        ...
        lmake = i .ne. 0
        if( lmake )then
c reset energy bounds, in case they were changed
...
c set table of svR values
          Q = dfcns( 1, icmp )
          do ir = 1, nrads
c choose R vals concentrated towards the ends of the range
            x = real( ir - 1 ) / real( nrads - 1 )
            x = .5 * ( 1.d0 - cos( pi * x ) )
            R = x * rtrunc( icmp )
            RShu( ir ) = R
            Om = Omegac( R )
            ak = akappa( R )
            if( Om .gt. 0.d0 )ak = min( ak, 2. * Om )
...
c central values
          ...
          Ftab( 1 ) = rhs( 1 ) / norm
          d( 1 ) = 1
c entropy penalty parameter - high value for a full-mass disk, decreasing
c   precipitously as the disk mass fraction drops
          upsilon = 3. + 8. * log10( cmpmas( icmp ) )
          upsilon = 10.**upsilon
          print *, 'disk mass fraction', cmpmas( icmp ),
     +             ' and upsilon', sngl( upsilon)
...
c set absolute and relative tolerances
          work( 31 ) = 1.d-8
          work( 32 ) = 1.d-5
!          print *, 'abs tol =', sngl( work( 31 ) ),
!     +             'rel tol =', sngl( work( 32 ) )
c least squares fitting routine
          call smsno( nval, d, Ftab, functn, iw, liw, lw, work,
     +                         uiparm, urparm, parmuf )
          if( master )then
            print *, 'DFSHU: return status for smsno', iw( 1 )
            if( iw( 1 ) .eq. 3 )then
              print *, '3 = x-convergence'
            else if( iw( 1 ) .eq. 4 )then
              print *, '4 = relative function convergence'
            else if( iw( 1 ) .eq. 5 )then
              print *, '5 = both x- and relative function convergence'
            else if( iw( 1 ) .eq. 6 )then
              print *, '6 = absolute function convergence'
            else if( iw( 1 ) .eq. 7 )then
              print *, '7 = singular convergence'
            else if( iw( 1 ) .eq. 8 )then
              print *, '8 = false convergence.  The iterates appear to'
     +                 // ' be converging to a noncritical point.'
            else if( iw( 1 ) .eq. 9 )then
              print *, '9 = function evaluation limit reached without'
     +                 // ' other convergence.'
            else if( iw( 1 ) .eq. 10 )then
              print *, '10 = iteration limit reached without other'
     +                // ' convergence'
            end if
            print *, 'For further interpretation of this return status'
     +           // ' see the source code for subroutine sumsl (not ' //
     +              'smsno) in GALAXY15/SRC/utils/Burkardt'
          end if
          if( iw( 1 ) .gt. 7 )then
              if( gtlogl( 'Do you want to quit' ) )
     +                   call crash( 'DFSHU', 'Failed to find minimum' )
          end if
...
c fit a 2D cubic spline
     ...
     +               Bc2, work, ifail )
       ...
       if( master )print *, 'DFSHU: Table ready'
        iuse = icmp
      end if
c default value
      dfShu = 0
