...
c set compressed halo
            ...
            temp( 2, icmp ) = hmass / extmas
            temp( 3, icmp ) = hscale / extscl
            temp( 4, icmp ) = fmass( icmp )
          end if
c report out what was read (or assumed)
          if( master )then
            print *
            if( lold )then
              print *,
     +           'Old compress.dat header information plus assumptions:'
            else
...
c read in compressed halo arrays
     ...
     +                      crash( 'COMPIN', 'Common arrays too small' )
          allocate ( arad( nradad ) )
          allocate ( phin( nradad ) )
          allocate ( gmn( nradad, ncomp ) )
          allocate ( rhon( nradad, ncomp ) )
          read( unit )( arad( i ), i = 1, nradad ),
     +                ( phin( i ), i = 1, nradad ),
     +                ( ( gmn( i, j ), i = 1, nradad ), j = 1, ncomp ),
     +                ( ( rhon( i, j ), i = 1, nradad ), j = 1, ncomp )
          if( lold )then
c may need to rescale old arrays
            if( ( extscl .ne. 1. ) .or. ( extmas .ne. 1. ) )then
              do i = 1, nradad
                arad( i ) = arad( i ) / extscl
                phin( i ) = phin( i ) * extscl / extmas
                do j = 1, ncomp
                  gmn( i, j ) = gmn( i, j ) / extmas
                  rhon( i, j ) = rhon( i, j ) * extscl**3 / extmas
                end do
              end do
            end if
          end if
...
c initialize spline tables
          ...
          allocate ( glamda( nradad + 4, ncomp ) )
          allocate ( gmnc( nradad + 4, ncomp ) )
          allocate ( rlamda( nradad + 4, ncomp ) )
          allocate ( rhonc( nradad + 4, ncomp ) )
          r = arad( 1 )
