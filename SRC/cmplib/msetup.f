...
c cc remove reading NCOM from input file --- PLi 2024/10/05
c      if( term )then
c        call gtintg( 'Enter number of mass components', ncmp )
c        if( master )print *, 'Setting up', ncmp, ' components'
c      else
c        call getline( ni, line )
c        if( line( 1:4 ) .ne. 'NCOM' )then
c          if( master )print *, 'Expected keyword: NCOM'
c          go to 1
c        end if
c        n = lnblnk( line )
c        read( line( 11:n ), *, err = 1 )ncmp
c        if( master )write( no, * )'Setting up', ncmp, ' components'
c      end if
      ncmp = 2
...
c cc Set inputs here rather than reading from files --PLi 2024/10/06
          if(icmp==1)then
                  line=' T '
          else if(icmp==2)then
                  line=' F '
...
c          call getline( ni, line )
c          if( line( 1:4 ) .ne. 'DISC' )then
c            if( master )print *, 'Expected keyword: DISC'
c            go to 1
c          end if
c          n = lnblnk( line )
          read( line( 1:3 ), *, err = 1 )disc( icmp )
c read disc/halo type
c cc making this block of code work for halo only --PLi 2024/10/06
          if(icmp==2)then
            call getline( ni, line )
            if( line( 1:4 ) .ne. 'TYPE' )then
              if( master )print *, 'Expected keyword: TYPE'
              go to 1
            end if
            n = lnblnk( line )
            m = 11
            do while ( line( m:m ) .eq. ' ' )
              m = m + 1
              if( m .gt. n )then
                if( master )print *, 'failed to find type word'
                go to 1
              end if
            end do
            type = line( m:m+3 )
            if(type=='EINA')then ! read kappa for EINA--PLi
               read(line(m+4:n), * , err=1)kappa
               DMhalo = 'EINA' 
            end if
          else if(icmp==1)then
            type='EXP'
          end if ! ending condition for halo only -- PLi 
        end if
c disk
        if( disc( icmp ) )then
          imtyp( icmp ) = 0
...
c flag model as not initialized
            rtidal = 0
          end if
...
c read mass for this component
          if( term )then
           call gtreal( 'Enter mass of this component', cmpmas( icmp ) )
          else
c cc only read data from file for halos --PLi
            if(icmp==1)then
                    cmpmas(icmp)=1.0 ! --PLi
            else if(icmp==2)then ! --PLi
                call getline( ni, line )
                if( line( 1:4 ) .ne. 'MASS' )then
                 if( master )print *, 'Expected keyword: MASS'
                  go to 1
                end if
                n = lnblnk( line )
...
c read radial scale for this component
        if( term )then
          call gtreal(
     +          'Enter radial scale of this component', rscale( icmp ) )
        else
c cc read only values from file for halo --PLi
          if(icmp==1)then
              rscale(icmp)=1.0
          else if(icmp==2)then
              call getline( ni, line )
              if( line( 1:4 ) .ne. 'SCAL' )then
                if( master )print *, 'Expected keyword: SCAL'
                go to 1
              end if
              n = lnblnk( line )
     ...
     +                        'Nonsense value of radial scale entered' )
        ...
        if( fixed )then
c set a notional truncation radius for fixed rotation curve models
c          rtrunc( icmp ) = 15
          rtrunc( icmp ) = 10
        else
c enter a truncation radius
          if( trnctd( icmp ) )then
            if( term )then
              call gtreal(
     +      'Enter truncation radius in scale lengths', rtrunc( icmp ) )
            else
c for some reason, err = 1 seemed not to trap an error on this read
              if(icmp==1)then ! set disk rtrunc here --PLi
                      cc=1.0
                      rtrunc(icmp)=5.0
              else if(icmp==2)then
                  read( line( 11:n ), *, iostat = i )cc, rtrunc( icmp )
                  if( i .ne. 0 )then
                    if( master )print *, 'line read error for rtrunc'
                    go to 1
                  end if
              end if
              if( master )write( no,
...
c DF type
        if( .not. term )then
          if(icmp==1)then ! set disk type here --PLi
                  type ='NONE'
          else if(icmp==2)then
              call getline( ni, line )
              if( line( 1:4 ) .ne. 'DFTY' )then
                if( master )print *, 'Expected keyword: DFTY'
                go to 1
              end if
              n = lnblnk( line )
              m = 11
              do while ( line( m:m ) .eq. ' ' )
                m = m + 1
                if( m .gt. n - 3 )then
                  if( master )print *, 'failed to find DFTYPE word'
                  go to 1
                end if
              end do
          type = line( m:m+3 )
...
c            do while ( line( m:m ) .ne. ' ' )
c              m = m + 1
c              if( m .gt. n )then
c                if( master )print *, 'initial Q missing?'
c                go to 1
c              end if
c            end do
c            read( line( m:n ), *, err = 1 )
c     +                                    ( dfcns( i, icmp ), i = 1, j )
            dfcns( 1, icmp )=0
...
c leave tapers turned off for now
...
c power law tapers for power law models
     ...
     +                                  // ' (suggest rmax - 5)', rtap )
                ...
                call getline( ni, line )
                if( line( 1:4 ) .ne. 'OUTT' )then
                  if( master )print *, 'Expected keyword: OUTT'
                  go to 1
                end if
                n = lnblnk( line )
...
c                call getline( ni, line )
c                if( line( 1:4 ) .ne. 'TAPE' )then
c                  if( master )print *, 'Expected keyword: TAPE'
c                  go to 1
c                end if
c                n = lnblnk( line )
                line =' f ' ! Set Lztapr here rather than from file --PLi
                read( line( 1:3 ), *, err = 1 )Lztapr( icmp )
c                if( Lztapr( icmp ) )read(
c     +                    line( 11:n ), *, err = 1 )Lztapr( icmp ), rtap
      ...
      if( froz )then
        ...
        do i = 1, ncmp
          if( ctype( i ) .ne. 'ASDI' )then
            if( disc( i ) )then
              if( ( m .gt. 0 ) )then
                if( ctype( i ) .ne. ctype( m ) )call crash( 'MSETUP',
...
c set variables in / model /
      call inimod
c initialize table - I moved this loop from before call inimod on 12/18/18
      do icmp = 1, ncmp
        if( .not. disc( icmp ) )cc = gmassh( 1.d0 )
      end do
c set outer boundary
      ...
      do icmp = 1, ncmp
        if( disc( icmp ) .and. ( cdft( icmp ) .eq. 'SHUE' ) )then
c still too soon to integrate over DF so force analytic expression
          dist( icmp ) = .false.
        end if
      end do
      call cutoff( cc )
...
c final call to cutoff now model is fully set up
      icmp = 0
      call cutoff( cc )
      return
