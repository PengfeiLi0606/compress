...
c P. Li made changes in the following two lines
c      if( ajrtab .eq. 0.d0 )call crash( 'AJRTAB', 'DB2VAL failed' )
c      if( ajrtab .eq. 0.d0 )print *, '#### Test #### E1, Emax(L1) = ',
c     +        E1, Emax(L1)
c      if( ajrtab .eq. 0.d0 )print *, '#### Test #### L, Lzmax(E) = ',
c     +        L, Lzmax(E)
      if( (ajrtab .eq. 0.d0) .and. (L - Lzmax( E ) .lt. 1.d-5))
     +       ajrtab = actj1( E, L )
