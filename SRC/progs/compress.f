...
c externals
      logical gtlogl
c  ########### naoise added  gammaP , cofa2 for einasto model ##############################
      real*8 cofa, distfn, E0tab, gmassh, gmdtab, gmext, Phihal, Phinx
      real*8 rhohal, Phitot,  gammaP, cofa2
c
c local variables
      character*20 args( 5 )
      integer i, j, k, nargs, status
      logical convrg, conserve, cuspy, line_arg
c      real a, hmass, hrad, x, y
c ######### naoise added local variable eina einx and gamma_inc for einasto model
      real a, x, y, eina, einx
      real*8 c, diffm, diffo, E, errmas, r, totmas, extmas, gamma_inc
      include 'inc/pi.f'
c
...
c set initial model
      ...
      call msetup
      print '( a, f10.4  )', 'shape parameter',  kappa
...
c compute concentration and v200
      ...
      if(DMhalo=='EINA')then
          a = sqrt( a )/2 ! hmass=16 pi rhos rs^3 for EINA --PLi
      else
          a = sqrt( a ) ! hmass=4 pi rhos rs^3 for NFW --PLi
      end if
c ########## naoise added cofa2 which is specific to Einasto model #####
      if(DMhalo=='EINA')then
          c = cofa2( dble( a ) ,dble(kappa))
      else
          c = cofa( dble( a ) )
      end if
      ...
      cNFW = c
      v200 = 0.7 * c * hrad
c ################ naoise added next lines to define einasto M200
      if(DMhalo=='EINA')then
         einx=  2.d0 *c**kappa /kappa
         eina = 3.d0/kappa
         gamma_inc = gammaP(dble(eina), dble(einx), 0)/4
         gamma_inc= gamma_inc *exp(2./kappa) 
         gamma_inc = (kappa/2.)**(3./kappa)*gamma_inc /kappa
         m200 = hmass * gamma_inc*gamma(dble(3/kappa)) 
      else
...
c initialize the main tables
      ...
      do i = 1, nradad
        ...
        gmn( i, 1 ) = gmassh( r )
        rhon( i, 1 ) = rhohal( r )
        if(rhon( i, 1 )==0.0D0)then
           rhon( i, 1 )=9.0D-323
           print*, 'rho = 0 encountered, new rho=', rhon( i, 1 )
        end if
...
c local variables
      ...
      character*10 hmass_str, hrad_str, ml_str
...
c create header for .out file
     ...
     +      status = 'unknown' )
      ...
      if( lbulge )then
        ...
        write( 2,
     +        '( ''# M_b/e10M_sun'', f10.4 )' )msb2( nvb ) / cmpmas( 1 )
...
c########## naoise edited NFW to Einasto in following line ##########
      if(DMhalo=='EINA')write( 2, '( ''# Einasto halo parameters'' )' )
c
c ######## naoise added next line since rs, mhalo has not been defined yet in this code
      rs = 1./ rscale(1)
      mhalo = 1./cmpmas(1)
...
c ######### naoise added one output line for einasto model shape parameter ###########
      if(DMhalo=='EINA')write( 2, '( ''# alpha'', f10.4 )' )kappa
c
      write( 2, '( ''#+++++++++++++++'' )' )
...
c ############ naoise changed name of output file since it caused some problems with consisitency with python ######## 
...
c ############### naoise: rest of code is added to accomadate Einasto model #######
c ###################################################################################################
      real*8 function cofa2( a, kappa1 )

c returns the value of c for a given A(c) for Einasto halos
c calling argument
      real*8 a, kappa1
c external
      real*8 aofc2
...
c
      ...
      do while ( ind .ne. 0 )
        call fndzro( c1, c2, res, tol, ir, w, ind, ifail )
        res = a- aofc2 (c1, kappa1)
      end do
      cofa2 = c1
      return
      end
c #############################################################################
c returns the function A(c) needed for the velocity scales of Einasto halos

      real*8 function aofc2( c, kappa2 )
c calling argument
      real*8 c, kappa2
c external
      real*8  gammaP
c local variable 
      real*8 gaminc, gam2, gam3
c
      gaminc = gammaP(3./kappa2, 2.*c**kappa2 /kappa2, 0)/kappa2
      gam2 = exp(2./kappa2) *(2./kappa2)**(-3./kappa2)*gaminc
      gam3 = gam2*gamma(3./kappa2)
      aofc2 = sqrt( c**3 / gam3 )
      return
      end
